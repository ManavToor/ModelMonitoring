...
def ya_standard_life(year_month: str = None, ref_ratio: float= 0.7) -> None:
    """
    run report on young adult standard life model
    ...
    # ref[target] = ref[target].replace({False: 0, True: 1})
    ref[target] = ref[target].astype('int')
    ref[target] = ref[target].astype('object')
    ref[prediction] = ref[prediction].astype('object')

    # cur[target] = cur[target].replace({False: 0, True: 1})
    cur[target] = cur[target].astype('int')
    ...
    for i in numerical:
        ref[i] = ref[i].astype('int64')
        cur[i] = cur[i].astype('int64')
